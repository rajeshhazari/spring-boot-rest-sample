...
#This script is extracted from github: https://github.com/ch4mpy/self-signed-certificate-generation
## This script is modified for my req to use in DOckerfile to create
## selfsigned cert and use with springboot rest api
## This script will generate selfsigned cert with all defaults to my api's api.c3transcribe
##
if [ -z "$SERVER_SSL_KEY_PASSWORD" ] || [ -z "$SERVER_SSL_KEY_STORE_PASSWORD" ]; then
    echo "following environment variables not set or (and identical):"

     SERVER_SSL_KEY_PASSWORD=2bcle@rS@cr@t
     SERVER_SSL_STORE_PASSWORD=2bcle@rS@cr@t

     echo "  SERVER_SSL_KEY_PASSWORD and SERVER_SSL_KEY_PASSWORD set with default value"
     echo "   SERVER_SSL_KEY_STORE_PASSWORD and SERVER_SSL_KEY_PASSWORD :: $SERVER_SSL_KEY_PASSWORD"
  #exit 1
...
CN=${HOSTNAME}
DEFAULT_ALTNAMES="localhost,devappserver-api.local,qaappserver-api.local"
CACERTS_PWD="changeit"
OUT_DIR="."
COUNTRY="US"
STATE="North Carolin"
CITY="Morrisville"
ORGANISATION="c3transcribe"
#EMAIL=`support`"@${ORGANISATION}.com"
EMAIL="support.c3transribe@gmail.com"
...
# Create templated self signed config
